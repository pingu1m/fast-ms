...
- 🎯 Entry points to execute your program using `python -m <fast_ms>` or `$ fast_ms` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/pingu1m/fast-ms/branch/main/graph/badge.svg?token=fast-ms_token_here)](https://codecov.io/gh/pingu1m/fast-ms)
[![CI](https://github.com/pingu1m/fast-ms/actions/workflows/main.yml/badge.svg)](https://github.com/pingu1m/fast-ms/actions/workflows/main.yml)

Awesome fast_ms created by pingu1m

## Install

from source
```bash
git clone https://github.com/pingu1m/fast-ms fast_ms
cd fast_ms
make install
```
...
```bash
$ fast_ms run --port 8080
```

or

```bash
python -m fast_ms run --port 8080
```
...
```bash
❯ fast_ms --help
Usage: fast_ms [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ fast_ms create-user --help
Usage: fast_ms create-user [OPTIONS] USERNAME PASSWORD
...
```bash
fast_ms create-user admin admin --superuser
```
...
Run with `fast_ms run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `fast_ms create-user`.
...
-----------------------------------------------------
fast_ms/__init__.py              4      0   100%
fast_ms/app.py                  16      1    94%
fast_ms/cli.py                  21      0   100%
fast_ms/config.py                5      0   100%
fast_ms/db.py                   10      0   100%
fast_ms/models/__init__.py       0      0   100%
fast_ms/models/content.py       47      1    98%
fast_ms/routes/__init__.py      11      0   100%
fast_ms/routes/content.py       52     25    52%
fast_ms/routes/security.py      15      1    93%
fast_ms/routes/user.py          52     26    50%
fast_ms/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in fast_ms/default.toml.
